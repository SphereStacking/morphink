...
<template>
  <Story name="Default" :args="{ label: 'Total Revenue', value: '$45,231' }">
    <template #template="{ args }">
      <Stat v-bind="args" />
    </template>
  </Story>

  <Story name="WithTrend" :args="{ label: 'Monthly Active Users', value: '2,350', trend: 'up', trendValue: '+12.5%', description: 'Compared to last month' }">
    <template #template="{ args }">
      <Stat v-bind="args" />
    </template>
  </Story>

  <Story name="StatsGrid" asChild>
