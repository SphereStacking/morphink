...
StyleDictionary.registerFormat({
  ...
  format: ({ dictionary }) => {
    ...
    for (const token of spaceTokens) {
      ...
      lines.push(
        `.ink-px-${key} { padding-left: var(${varName}); padding-right: var(${varName}); }`
      )
      lines.push(
        `.ink-py-${key} { padding-top: var(${varName}); padding-bottom: var(${varName}); }`
      )
      ...
      lines.push(
        `.ink-space-x-${key} > :not([hidden]) ~ :not([hidden]) { --tw-space-x-reverse: 0; margin-right: calc(var(${varName}) * var(--tw-space-x-reverse)); margin-left: calc(var(${varName}) * calc(1 - var(--tw-space-x-reverse))); }`
      )
      lines.push(
        `.ink-space-y-${key} > :not([hidden]) ~ :not([hidden]) { --tw-space-y-reverse: 0; margin-top: calc(var(${varName}) * calc(1 - var(--tw-space-y-reverse))); margin-bottom: calc(var(${varName}) * var(--tw-space-y-reverse)); }`
      )
    }

    lines.push(
      '.ink-space-x-reverse > :not([hidden]) ~ :not([hidden]) { --tw-space-x-reverse: 1; }'
    )
    lines.push(
      '.ink-space-y-reverse > :not([hidden]) ~ :not([hidden]) { --tw-space-y-reverse: 1; }'
    )
    ...
    for (const token of radiusTokens) {
      ...
      lines.push(
        `.ink-rounded-t-${key} { border-top-left-radius: var(${varName}); border-top-right-radius: var(${varName}); }`
      )
      lines.push(
        `.ink-rounded-b-${key} { border-bottom-left-radius: var(${varName}); border-bottom-right-radius: var(${varName}); }`
      )
      lines.push(
        `.ink-rounded-l-${key} { border-top-left-radius: var(${varName}); border-bottom-left-radius: var(${varName}); }`
      )
      lines.push(
        `.ink-rounded-r-${key} { border-top-right-radius: var(${varName}); border-bottom-right-radius: var(${varName}); }`
      )
